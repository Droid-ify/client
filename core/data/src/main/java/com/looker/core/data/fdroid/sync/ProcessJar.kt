...
suspend fun JarFile.getFingerprint(): String = suspendCancellableCoroutine { cont ->
	...
	if (codeSigners == null || codeSigners.size != 1) throw ProcessJarException("index.jar must be signed by a single code signer")
	val certificates = codeSigners[0].signerCertPath?.certificates.orEmpty()
	if (certificates.size != 1) throw ProcessJarException("index.jar code signer should have only one certificate")
